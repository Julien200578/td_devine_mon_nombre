...
....
🡆 Répondre ici 🡄
il compile le fichier devine_mon_nombre
....
...
....
🡆 Répondre ici 🡄
git tag -a v1.0.0 -m "TP 'Devine mon nombre !' — fin de la première partie (v1.0.0)"
....
...
🡆 Répondre ici 🡄
clippy fait généralement des remarques de style ou d’optimisation
...
+
- Les binaires sont placés dans `target/release/` au lieu de `target/debug/`.
- Le binaire est beaucoup plus optimisé (plus rapide à l’exécution).
- La taille du binaire peut être plus grande ou plus petite selon les optimisations appliquées.
...
....
🡆 Répondre ici 🡄
La bibliothèque `console` fournit des utilitaires pour travailler avec la console : gestion des couleurs et styles de texte, contrôle du curseur, détection de la taille du terminal, saisie masquée, ainsi que des outils pratiques comme un chronomètre. Elle sert donc à améliorer l’affichage et les interactions en ligne de commande.
...
....
🡆 Répondre ici 🡄
git tag -a v1.1.0 -m "TP 'Devine mon nombre !' — version finale avec tests et console"
